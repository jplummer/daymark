...
- **Automated tests:** `npm test` runs Vitest. `src/live-preview.ordered-filter.test.ts` covers the transaction filter and a few helpers using `EditorState` + `state.update` (no DOM). Multi-step editor flows (paste → Enter → Shift-Tab → type) are not in CI yet; those would need either an `EditorView` + simulated keys in jsdom/happy-dom, or Playwright against `vite preview` / the Tauri build.
